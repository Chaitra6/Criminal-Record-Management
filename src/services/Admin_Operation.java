...
public class Admin_Operation {
	ArrayList<Criminal_Details> criminalList = new ArrayList<Criminal_Details>();
...
//				while(flag == 1) {
//				
//				for(Criminal_Details cd : criminalList) { 
...
//				}
			...
			case 3: {
				...
				System.out.println(criminalList.get(id - 1).getCriminalID() +"\t"
			...
			case 4:{
				AdminViewCriminals ad = new AdminViewCriminals();
			...
			case 5: {
				...
				int id = Integer.parseInt(br.readLine());
				for(Criminal_Details cd : criminalList) { 					
					   if(cd.getCriminalID() == id) { 
					       criminalList.remove(cd);
					   }					   
					}
